...
```bash
# Debian/Ubuntu
sudo apt install build-essential pkg-config libudev-dev libhidapi-dev libgtk-3-dev
...
in `atk-sys`, `gdk-pixbuf-sys`, and `glib-sys` on Linux. Ubuntu
Desktop boxes typically already have the runtime libs from a
